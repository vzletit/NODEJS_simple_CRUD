...
export default () => {
  const methods: any = {
    GET: (req: Req, res: Res, urlParam?: string) =>
      GET(req, res, db, config, urlParam),
    POST: (req: Req, res: Res, urlParam?: string) =>
      POST(req, res, db, config, urlParam),
    PUT: (req: Req, res: Res, urlParam?: string) =>
      PUT(req, res, db, config, urlParam),
    DELETE: (req: Req, res: Res, urlParam?: string) =>
      DELETE(req, res, db, config, urlParam)
  };

  const server = http.createServer(
    (req: IncomingMessage, res: ServerResponse) => {
      const parsedURL: Array<string> = req
        .url!.split('/')
        .filter((path) => path);
